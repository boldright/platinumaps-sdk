...
| `safeAreaTopOverride` | `Int?` | — | Optional override for the top safe-area inset reported to the web layer. The SDK reads `safeAreaInsets.top` after the first layout pass; set the override when the host already knows the value. |
