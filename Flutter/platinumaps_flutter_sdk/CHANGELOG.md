...
- `safeAreaTop` / `safeAreaBottom` default to `null`, which resolves
  ...
  creation: the device inset for an edge-to-edge map, `0` below an
  opaque `AppBar`. Note `extendBodyBehindAppBar: true` inflates the
  ambient padding to the AppBar's bottom edge, so a full-screen map
  under such an `AppBar` should pass the route-level
  `MediaQuery.of(context).padding` explicitly. The resolved value is
  forwarded to the native side verbatim, so native behaviour is
  unchanged.
