...
/// Delegate that the host application may set on `PMMapView` to take over the
/// handling of links that the embedded Platinumaps web app wants to open.
/// When no delegate is set the SDK falls back to `SFSafariViewController` for
/// HTTPS/HTTP and `UIApplication.open` for known custom schemes.
///
...
///
/// Lifecycle: the view wires itself as the `WKNavigationDelegate`,
/// `WKUIDelegate`, and `CLLocationManagerDelegate`. It registers
/// `UIApplication.willEnterForeground` / `didEnterBackground` observers on
/// first attachment to a window and tears them down in `deinit`. All public
/// state is expected to be read or written on the main actor.
///
/// Bridge protocol:
...
public class PMMapView: UIView {
    ...
    public weak var delegate: PMMapViewDelegate?
    ...
    /// Latch used to perform first-attach setup exactly once, even though
    /// `didMoveToWindow` may fire multiple times as the view moves in and
    ...
    /// Timestamp captured at view instantiation. Used to enforce a minimum
    /// cover-image display window so the splash does not flash.
...
extension UIView {
    ...
    fileprivate var presentationViewController: UIViewController? {
...
extension PMMapView: WKNavigationDelegate {
...
extension PMMapView {
    ...
    fileprivate static func jsLiteral(_ value: Any) -> String? {
        do {
            ...
            NSLog("PMMapView.jsLiteral failed: \(error)")
            return nil
        }
    }
...
extension PMMapView: @preconcurrency CLLocationManagerDelegate {
    ...
    private func localizedString(forKey key: String) -> String {
        return PMLocalizedStrings.string(forKey: key)
    }
...
extension PMMapView {
    ...
    public func pushLaunchURL(_ url: URL) {
        ...
        // Locate the owning view controller (without walking past it into
        // the presented chain) so we can dismiss the entire modal stack it
        // has spawned and land the user back on the map.
        var responder: UIResponder? = self
        var owner: UIViewController?
        while let r = responder {
            if let vc = r as? UIViewController { owner = vc; break }
...
extension PMMapView {
    ...
    public func locationManager(_ manager: CLLocationManager, didDetermineState state: CLRegionState, for inRegion: CLRegion) {
        guard useBeacon == true else {
            return
        }
...
//   * Retries continue forever while the view is foregrounded. The
