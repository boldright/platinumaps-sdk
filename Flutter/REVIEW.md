...
- ❌ **#25** iOS Plugin の SwiftPM library 名がハイフン (`platinumaps-flutter-sdk`) — `Flutter/platinumaps_flutter_sdk/ios/platinumaps_flutter_sdk/Package.swift:42`
  - 当初 underscore に揃えたが CI で `product 'platinumaps-flutter-sdk' ... not found in package 'platinumaps_flutter_sdk'` が発生。Flutter の SwiftPM プラグイン規約「plugin 名に `_` を含むときは library 名を `-` 区切りにする」（`FlutterGeneratedPluginSwiftPackage` が hyphen 形式で参照する）が正解で、レビュー指摘自体が誤りだったため hyphen を維持し受容。
