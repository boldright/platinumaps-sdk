...
| 6 | Test suite: Dart unit tests + native plugin-glue tests + `integration_test` driven from the example app | Tests green on CI matrix (§7) | 🟡 all three test layers in place (Dart `flutter test` covers the public API; Android JUnit covers `PlatinumapsPlatformView.buildMapOptions`; iOS XCTest covers `applyCreationArguments(_:to:)` via the example app's `RunnerTests` target; `integration_test/` boots the example on a real device). CI matrix is still pending. |
